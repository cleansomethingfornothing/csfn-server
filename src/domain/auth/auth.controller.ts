...
export class AuthController {
    ...
    @Post('register')
    @Validation([CREATE])
    register(@Session() session: any, @Headers('country') country: string, @Body() user: User): Promise<User> {
    ...
    @Basic()
    @HttpCode(200)
    login(@Session() session: any, @Req() req) {
        session.user = {id: req.user.id}
        return req.user
    }

    @Post('login_facebook')
    facebookLogin(@Req() req: Request, @Session() session, @Headers('country') country: string, @Body() {token}: { token: string }) {
        return this.loginSocial(session, token, this.authService.loginWithFacebook(token, country))
    }

    @Post('login_google')
    googleLogin(@Session() session, @Headers('ios') ios: string, @Headers('country') country: string, @Body() {token}: { token: string }) {
        return this.loginSocial(session, token, this.authService.loginWithGoogle(token, ios === 'true', country))
    }

    private loginSocial(session, token, promise: Promise<[User, boolean]>): Promise<User> {
        if (!token) {
    ...
    changeEmail(@Req() req, @Body() {email}: User) {
    ...
    changePassword(@Req() req, @Body() {password}: User) {
    ...
    @Basic()
    @HttpCode(200)
    delete(@Session() session: any, @Req() req, @Res() res: Response) {
