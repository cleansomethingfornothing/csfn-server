...
export class UsersService {
  ...
  findOneByEmail({ email, facebookId, googleId }: { email: string, facebookId?: string, googleId?: string }): Promise<User> {
    return this.userRepository.findOneOrFail({
      where: [{ email }, { facebookId }, { googleId }],
      relations: ['picture']
    })
  }

  findTopUsers(userCountry: string,
               sortBy: 'totalWeight' | 'totalVolume'): Promise<any> {
  ...
  removePreviousPictureIfChanged(id: number,
                                 current: User,
                                 update: User) {
