...
export class CleanupsController {
    ...
    @Logged()
    @Validation([CREATE])
    register(@Session() session: any, @Body() cleanup: Cleanup) {
    ...
    update(@Session() session: any, @Param('id') id: number, @Body() cleanup: Cleanup) {
        return this.cleanupsService.findOne(id)
    ...
    @Logged()
    @Delete(':id')
    delete(@Session() session: any, @Param('id') id: number) {
        return this.cleanupsService.findOne(id)
            .catch(() => Promise.reject(new NotFoundException()))
